...
namespace CustomCalc {
    class RPN {
        ...
        int dvd = 7; // Изменено на 7 для семеричной системы
        int lb = 47;
        int rb = 55; // Изменено на 55 для семеричной системы
        ulong ns = 7; // Изменено на 7 для семеричной системы
   ...
   public void getInput(char[] input) {
    ...
    for (int i = 0; i < input.Length; ++i) {
        ...
        if (char.IsDigit(cur) || cur == '.') {
            ...
            if (number.Length > 0) {
                ...
                //this.list.Add(getNum(number, isPositive));
                isPositive = true;
            }
    ...
    addOp('+');
		...
		private void addOp(char op) {
			char lastOp;
			if (this.ops.Count == 0) { // Проверяем, пуст ли стек
				this.ops.Push(op);
				return;
			}
			lastOp = this.ops.Peek(); // Получаем верхний элемент без удаления
			if (op_priority(op) > op_priority(lastOp)) {
				if (op == ')') {
					while (lastOp != '(') {
						this.list.Add(lastOp);
						this.ops.Pop(); // Удаляем элемент из стека
						if (this.ops.Count == 0) { // Проверяем, пуст ли стек
							break;
						}
						lastOp = this.ops.Peek(); // Получаем новый верхний элемент
					}
					this.ops.Pop(); // Удаляем '(' из стека
				} else {
					this.ops.Push(op);
				}
				return;
...
public char[] convertDecToDNS(double num, ulong ns) {
	...
	i = 0;
	if (fracp > eps) {
		fracPart[i++] = '.'; // Добавляем символ запятой
		for (; i < 5; ++i) {
			fracp *= ns;
	...
	if (fracp > eps) {
		res[i++] = '.'; // Добавляем символ запятой
		while (i < res.Length) {
	...
	return res;
        ...
        public double convertDNSToDec(char[] num, ulong ns) {
	        ...
	        //int mInd = 12;
	        //mInd = Math.Min(num.Length - k - 1, 14);
	        ...
	        //else if (mInd + k + 1 < num.Length && num[num.Length - 1] == '-') {
	        //	res *= -1;
	        //}
	        return res;
        }
